...
void PC_Check_Tilecollision() {
  ...
  if (PC_Collision_Right()) {
    PC.RunVelocity = 0;
    PC.HitWall = true;
    if (!GV.PixelPerfectRunning) {
      PC.PosX -= ((PC.PosX + PC.ColWidthHalf) % TS.Tile_Width);
    }  // NOT NEEDED ANYMORE SINCE MOVEMENT IS PIXEL PERFECT
  }
  ...
  if (PC_Collision_Left()) {
    ...
    if (!GV.PixelPerfectRunning) {
      PC.PosX +=
          TS.Tile_Width - 1 - ((PC.PosX - PC.ColWidthHalf) % TS.Tile_Width);
    }  // NOT NEEDED ANYMORE SINCE MOVEMENT IS PIXEL PERFECT
  }
...
bool PC_Collision_Down() {
  ...
  if (PC.JumpVelocity < 0 && PC.RunVelocity == 0) {
    if (TileType[StageC64.TileNumber[(int)((PC.PosX - PC.ColWidthHalf + 1) /
                                           TS.Tile_Width)]
                                    [(int)(PC.PosY / TS.Tile_Height)]]
            .Solid ||  // CHECK BOTTOM-LEFT
        TileType[StageC64.TileNumber[(int)((PC.PosX - PC.ColWidthQuart) /
                                           TS.Tile_Width)]
                                    [(int)(PC.PosY / TS.Tile_Height)]]
            .Solid ||
        TileType[StageC64.TileNumber[(int)(PC.PosX / TS.Tile_Width)]
                                    [(int)(PC.PosY / TS.Tile_Height)]]
        ...
        TileType[StageC64.TileNumber[(int)((PC.PosX + PC.ColWidthQuart) /
                                    ...
                                    [(int)(PC.PosY / TS.Tile_Height)]]
            .Solid ||
        TileType[StageC64.TileNumber[(int)((PC.PosX + PC.ColWidthHalf - 1) /
                                           TS.Tile_Width)]
                                    [(int)(PC.PosY / TS.Tile_Height)]]
  ...
  if (PC.JumpVelocity < 0 && PC.RunVelocity < 0) {
    if (TileType[StageC64.TileNumber[(int)((PC.PosX - PC.ColWidthQuart) /
                                           TS.Tile_Width)]
                                    [(int)(PC.PosY / TS.Tile_Height)]]
            .Solid ||
        TileType[StageC64.TileNumber[(int)(PC.PosX / TS.Tile_Width)]
  ...
  if (PC.JumpVelocity < 0) {
    if (TileType[StageC64.TileNumber[(int)((PC.PosX - PC.ColWidthQuart) /
        ...
        TileType[StageC64.TileNumber[(int)((PC.PosX + PC.ColWidthHalf - 1) /
            ...
            .Solid)  // CHECK BOTTOM-RIGHT
    {
      Check_Switch_Tile_Down(
          (int)((PC.PosX - PC.ColWidthHalf + 1) / TS.Tile_Width),
          (int)(PC.PosY / TS.Tile_Height));
      Check_Switch_Tile_Down((int)((PC.PosX) / TS.Tile_Width),
                             (int)(PC.PosY / TS.Tile_Height));
      Check_Switch_Tile_Down(
          (int)((PC.PosX + PC.ColWidthHalf - 1) / TS.Tile_Width),
          (int)(PC.PosY / TS.Tile_Height));
  ...
  if (PC.PosY < 0) {
    ReturnValue = false;
  }  // DON'T GET STUCK ON TOP OF SCREEN
...
bool PC_Collision_Right() {
  bool ReturnValue = false;
  if (PC.RunVelocity > 0 && (PC.PosY > PC.ColHeight ||
                             !GV.PixelPerfectRunning))  // RUNNING TO THE RIGHT
  {
    if (TileType[StageC64.TileNumber[(int)((PC.PosX + PC.ColWidthHalf) /
                                           TS.Tile_Width)][(
                     int)((PC.PosY - PC.ColHeightHalf) / TS.Tile_Height)]]
            .Solid ||  // CHECK CENTER-RIGHT
  ...
  for (i = 1; i < 30; i++) {
    if (Sprite_DropStone[i].inUse) {
      if (PC.PosX + PC.ColWidthHalf - 1 > Sprite_DropStone[i].PosX &&
          PC.PosX + PC.ColWidthHalf - 1 <
              Sprite_DropStone[i].PosX + Sprite_DropStone[i].w &&
...
bool PC_Collision_Left() {
  bool ReturnValue = false;
  if (PC.RunVelocity < 0 && (PC.PosY > PC.ColHeight ||
                             !GV.PixelPerfectRunning))  // RUNNING TO THE LEFT
  {
    if (TileType[StageC64.TileNumber[(int)((PC.PosX - PC.ColWidthHalf) /
                                           TS.Tile_Width)][(
                     int)((PC.PosY - PC.ColHeightHalf) / TS.Tile_Height)]]
            .Solid ||  // CHECK CENTER-LEFT
        TileType[StageC64.TileNumber[(int)((PC.PosX - PC.ColWidthHalf) /
                                           TS.Tile_Width)]
                                    [(int)((PC.PosY - 1) / TS.Tile_Height)]]
            .Solid)  // CHECK BOTTOM-LEFT
  ...
  for (i = 1; i < 30; i++) {
    ...
    if (Sprite_DropStone[i].inUse) {
      if (PC.PosY > Sprite_DropStone[i].PosY - 1 &&
          ...
          PC.PosX - PC.ColWidthHalf + 3 <
              Sprite_DropStone[i].PosX + Sprite_DropStone[1].w) {
        ReturnValue = true;
        StonePCStandsOn = i;
      }
