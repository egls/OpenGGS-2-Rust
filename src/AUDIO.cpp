...
void AUDIO_Define() {
  ...
  AUDIO_CHUNK_Disk =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_DISK]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Jump);
  AUDIO_CHUNK_Jump = NULL;
  AUDIO_CHUNK_Jump =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_JUMP]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Stomp);
  AUDIO_CHUNK_Stomp = NULL;
  AUDIO_CHUNK_Stomp =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_STOMP]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Killed);
  AUDIO_CHUNK_Killed = NULL;
  AUDIO_CHUNK_Killed =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_KILLED]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Ding);
  AUDIO_CHUNK_Ding = NULL;
  AUDIO_CHUNK_Ding =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_DING]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Click);
  AUDIO_CHUNK_Click = NULL;
  AUDIO_CHUNK_Click =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_CLICK]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_GotPowerUp);
  AUDIO_CHUNK_GotPowerUp = NULL;
  AUDIO_CHUNK_GotPowerUp =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_GOTPOWERUP]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Shoot);
  AUDIO_CHUNK_Shoot = NULL;
  AUDIO_CHUNK_Shoot =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_SHOOT]);  //load the chunks

  Mix_FreeChunk(AUDIO_CHUNK_Crack);
  AUDIO_CHUNK_Crack = NULL;
  AUDIO_CHUNK_Crack =
      Mix_LoadWAV(FileName.WavChunk[AUDIO_CRACK]);  //load the chunks
...
void AUDIO_Music_Play(int MusicNumber) {
  if (GV.VolumeMusic > 0 && GV.MusicEnabled) {
    ...
    if (MusicNumber == MUSIC_MENU) {
      Mix_PlayMusic(BGM_Title, -1);
    }
    if (MusicNumber == MUSIC_OUTDOORS) {
      Mix_PlayMusic(BGM_Outdoors, -1);
    }
    if (MusicNumber == MUSIC_INDOORS) {
