...
void LOOP_Editor_Stages_Draw_Enemies() {
  ...
  for (x = 0; x < MAX_NUM_ENEMIES; x++) {
    if (ME.Enemy_InUse[x]) {
      LOOP_Editor_Stages_Draw_Enemy(x);
    }
  }
}
...
void LOOP_Editor_Stages_Set_New_Enemy(int EnemyType, int x, int y) {
  ...
  {
    if (ME.Enemy_PosX[i] == x && ME.Enemy_PosY[i] == y && ME.Enemy_InUse[i]) {
...
void LOOP_Editor_Stages_Draw_Enemy(int EnemyNumber) {
  ...
  if (ME.Enemy_Type[EnemyNumber] == 15) {
  ...
  if (ME.Enemy_InUse[EnemyNumber]) {
    ...
    if (ME.Enemy_Type[EnemyNumber] < 16) {
      SrcRect.x = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                      .FrameX[ME.Enemy_Direction[EnemyNumber]][AnimationFrame]
                             [NPC_ALIVE];
      SrcRect.y = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                      .FrameY[ME.Enemy_Direction[EnemyNumber]][AnimationFrame]
                             [NPC_ALIVE];
      SrcRect.w = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                      .FrameW[ME.Enemy_Direction[EnemyNumber]][AnimationFrame]
                             [NPC_ALIVE];
      SrcRect.h = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                      .FrameH[ME.Enemy_Direction[EnemyNumber]][AnimationFrame]
                             [NPC_ALIVE];

      if (!ME.Enemy_Alive[EnemyNumber]) {
        SrcRect.x = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                        .FrameX[NPC_DEAD][NPC_DEAD][NPC_DEAD];
        SrcRect.y = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                        .FrameY[NPC_DEAD][NPC_DEAD][NPC_DEAD];
        SrcRect.w = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                        .FrameW[NPC_DEAD][NPC_DEAD][NPC_DEAD];
        SrcRect.h = Enemy_Type[ME.Enemy_Type[EnemyNumber]]
                        .FrameH[NPC_DEAD][NPC_DEAD][NPC_DEAD];
      }
    }

    DstRect.w = SrcRect.w;
    DstRect.h = SrcRect.h;
    DstRect.x = (ME.Enemy_PosX[EnemyNumber] * TS.Tile_Width) -
                (int)(SrcRect.w / 2) - ME.StagePosX;
    ...
    int GrabberY =
        (ME.Enemy_PosY[EnemyNumber] * TS.Tile_Height) - TS.Tile_Height;
    if (EnemyNumber == ME.CurrentEnemy) {
      INTERFACE_Tile_Draw(GrabberX, GrabberY, INTERFACE_MARK_GRAB_ACTIVE);
    }
    if (EnemyNumber != ME.CurrentEnemy) {
      INTERFACE_Tile_Draw(GrabberX, GrabberY, INTERFACE_MARK_GRAB);
    }
