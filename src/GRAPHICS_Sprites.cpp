...
void SPRITES_Define() {
  ...
  if (bonusfile == NULL) {
    ...
    for (x = 0; x < 40; x++) {
      fscanf(bonusfile, "%i,", &temparray[x]);
    }
    x = 0;

    for (i = 1; i < 9; i++) {
      Sprite_PowerUp[i].x = temparray[x];
      x++;
      Sprite_PowerUp[i].y = temparray[x];
      x++;
      Sprite_PowerUp[i].w = temparray[x];
      x++;
      Sprite_PowerUp[i].h = temparray[x];
      x++;
    ...
    Sprite_Bullet.x = temparray[x];
    x++;
    Sprite_Bullet.y = temparray[x];
    x++;
    Sprite_Bullet.w = temparray[x];
    x++;
    Sprite_Bullet.h = temparray[x];
    x++;
