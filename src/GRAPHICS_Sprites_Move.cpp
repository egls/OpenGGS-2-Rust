...
void SPRITES_Move() {
  int i;
  for (i = 1; i < 20;
       i++)  // DON'T MESS WITH SMALLCOIN[0] - IT'S USED IN THE INTERFACE
  {
  ...
  for (i = 1; i < 10; i++) {
    if (Sprite_PowerUp[i].Active) {
      ...
      if (Sprite_PowerUp[i].OnGround) {
        Sprite_PowerUp_Walk(i);
      }
      //Sprite_PowerUp_Walk(i);
      if (!Sprite_PowerUp[i].OnGround) {
        Sprite_PowerUp_Gravity(i);
      }
...
void Sprite_SmallCoin_Gravity(int SmallCoinNumber) {
  ...
  Sprite_SmallCoin[SmallCoinNumber].PosY -=
      int(Sprite_SmallCoin[SmallCoinNumber].JumpVelocity / 2);

  if (Sprite_SmallCoin[SmallCoinNumber].PosY > GV.Screen_Height) {
    Sprite_SmallCoin[SmallCoinNumber].Active = false;
  }
...
void Sprite_PowerUp_Gravity(int PowerUpNumber) {
  ...
  Sprite_PowerUp[PowerUpNumber].PosY -=
      int(Sprite_PowerUp[PowerUpNumber].JumpVelocity / 2);

  if (Sprite_PowerUp[PowerUpNumber].JumpVelocity < -World.TerminalVelocity) {
    Sprite_PowerUp[PowerUpNumber].JumpVelocity = -World.TerminalVelocity;
  }
}
...
void Sprite_PowerUp_Tilecollision(int PowerUpNumber) {
  ...
  if (Sprite_PowerUp_Collision_Down(PowerUpNumber)) {
    // BOUNCE POWERUP
    if (Sprite_PowerUp[PowerUpNumber].JumpVelocity < -8) {
      Sprite_PowerUp[PowerUpNumber].JumpVelocity =
          -Sprite_PowerUp[PowerUpNumber].JumpVelocity - 8;
    }

    Sprite_PowerUp[PowerUpNumber].OnGround = true;

    // PREVENT SINKING INTO TILE
    Sprite_PowerUp[PowerUpNumber].PosY -= ((Sprite_PowerUp[PowerUpNumber].PosY +
                                            Sprite_PowerUp[PowerUpNumber].h) %
                                           TS.Tile_Height);
  }
  ...
  if (Sprite_PowerUp[PowerUpNumber].Direction == NPC_LEFT) {
    ...
    if (Sprite_PowerUp_Collision_Right(PowerUpNumber) ||
        Sprite_PowerUp[PowerUpNumber].PosX >
            StageC64.StageWidthPixels - Sprite_PowerUp[PowerUpNumber].w) {
...
void Sprite_PowerUp_Walk(int PowerUpNumber) {
  ...
  if (Sprite_PowerUp[PowerUpNumber].Direction == NPC_RIGHT) {
    Sprite_PowerUp[PowerUpNumber].RunVelocity += 1 * World.Friction;
  }
  if (Sprite_PowerUp[PowerUpNumber].Direction == NPC_LEFT) {
    Sprite_PowerUp[PowerUpNumber].RunVelocity -= 1 * World.Friction;
  }
  ...
  // MAX SPEED
  if (Sprite_PowerUp[PowerUpNumber].RunVelocity <
      -Sprite_PowerUp[PowerUpNumber].RunStrength) {
    Sprite_PowerUp[PowerUpNumber].RunVelocity =
        -Sprite_PowerUp[PowerUpNumber].RunStrength;
  }  // MAX SPEED
  if (Sprite_PowerUp[PowerUpNumber].RunVelocity >
      Sprite_PowerUp[PowerUpNumber].RunStrength) {
  ...
  Sprite_PowerUp[PowerUpNumber].PosX +=
      int(Sprite_PowerUp[PowerUpNumber].RunVelocity / 4);
...
bool Sprite_PowerUp_Collision_Down(int PowerUpNumber) {
  ...
  if (Sprite_PowerUp[PowerUpNumber].JumpVelocity < 0) {
    if (TileType[StageC64.TileNumber[(int)(Sprite_PowerUp[PowerUpNumber].PosX /
                                           TS.Tile_Width)]
                                    [(int)((Sprite_PowerUp[PowerUpNumber].PosY +
                                            Sprite_PowerUp[PowerUpNumber].h) /
                                           TS.Tile_Height)]]
            .Solid ||  // CHECK BOTTOM-LEFT
        TileType[StageC64
                     .TileNumber[(int)((Sprite_PowerUp[PowerUpNumber].PosX +
                                        Sprite_PowerUp[PowerUpNumber].w / 2) /
                                       TS.Tile_Width)]
                                [(int)((Sprite_PowerUp[PowerUpNumber].PosY +
                                        Sprite_PowerUp[PowerUpNumber].h) /
                                       TS.Tile_Height)]]
            .Solid ||  // CHECK BOTTOM-CENTER
        TileType[StageC64.TileNumber[(int)((Sprite_PowerUp[PowerUpNumber].PosX +
                                            (Sprite_PowerUp[PowerUpNumber].w)) /
                                           TS.Tile_Width)]
                                    [(int)((Sprite_PowerUp[PowerUpNumber].PosY +
                                            Sprite_PowerUp[PowerUpNumber].h) /
                                           TS.Tile_Height)]]
            .Solid)  // CHECK BOTTOM-RIGHT
    {
      ReturnValue = true;
    }
  }
...
bool Sprite_PowerUp_Collision_Left(int PowerUpNumber) {
  ...
  if (TileType[StageC64.TileNumber[(int)(Sprite_PowerUp[PowerUpNumber].PosX /
                                         TS.Tile_Width)][(
                   int)(Sprite_PowerUp[PowerUpNumber].PosY / TS.Tile_Height)]]
          .Solid)  // CHECK TOP-LEFT
  {
    ReturnValue = true;
  }
...
void Sprite_Platform_Move() {
  //if(Platform.PosX - PC.PosX < 640)
  {
    if (Platform.Direction == 1) {
      if (Platform.PosY < Platform.End_Y) {
        Platform.PosY++;
      }
      if (Platform.PosY > Platform.End_Y) {
        Platform.PosY--;
    ...
    if (Platform.Direction == 0) {
      if (Platform.PosY < Platform.Start_Y) {
        Platform.PosY++;
      }
      if (Platform.PosY > Platform.Start_Y) {
        Platform.PosY--;
...
void SPRITE_DropStone_Gravity(int DropStoneNumber) {
  ...
  if (Sprite_DropStone[DropStoneNumber].JumpVelocity < 0) {
    if (TileType[StageC64
                     .TileNumber[(int)(Sprite_DropStone[DropStoneNumber].PosX /
                                       TS.Tile_Width)]
                                [(int)((Sprite_DropStone[DropStoneNumber].PosY +
                                        Sprite_DropStone[DropStoneNumber].h) /
                                       TS.Tile_Height)]]
            .Solid ||  // CHECK BOTTOM-LEFT
        TileType[StageC64.TileNumber[(
                     int)((Sprite_DropStone[DropStoneNumber].PosX +
                           Sprite_DropStone[DropStoneNumber].w / 2) /
                          TS.Tile_Width)][(
                     int)((Sprite_DropStone[DropStoneNumber].PosY +
                           Sprite_DropStone[DropStoneNumber].h) /
                          TS.Tile_Height)]]
        ...
        TileType[StageC64
                     .TileNumber[(int)((Sprite_DropStone[DropStoneNumber].PosX +
                                        (Sprite_DropStone[DropStoneNumber].w)) /
                                       TS.Tile_Width)]
                                [(int)((Sprite_DropStone[DropStoneNumber].PosY +
                                        Sprite_DropStone[DropStoneNumber].h) /
                                       TS.Tile_Height)]]
            .Solid)  // CHECK BOTTOM-RIGHT
    {
      ...
      Sprite_DropStone[DropStoneNumber].PosY -=
          ((Sprite_DropStone[DropStoneNumber].PosY +
            Sprite_DropStone[DropStoneNumber].h) %
           TS.Tile_Height);
    }
  ...
  Sprite_DropStone[DropStoneNumber].PosY -=
      int(Sprite_DropStone[DropStoneNumber].JumpVelocity / 2);

  if (Sprite_DropStone[DropStoneNumber].JumpVelocity <
      -World.TerminalVelocity) {
    Sprite_DropStone[DropStoneNumber].JumpVelocity = -World.TerminalVelocity;
  }
}
