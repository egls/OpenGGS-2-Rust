...
void PC_Check_Collision_PowerUp(int PowerUpNumber) {
  ...
  int bx2 =
      Sprite_PowerUp[PowerUpNumber].PosX + Sprite_PowerUp[PowerUpNumber].w;
  int by1 = Sprite_PowerUp[PowerUpNumber].PosY;
  int by2 =
      Sprite_PowerUp[PowerUpNumber].PosY + Sprite_PowerUp[PowerUpNumber].h;

  if (bx1 > PC.x1 && bx1 < PC.x3 && by1 > PC.y1 && by1 < PC.y3) {
    PowerUpCollected = true;
  }
  if (bx2 > PC.x1 && bx2 < PC.x3 && by2 > PC.y1 && by2 < PC.y3) {
    PowerUpCollected = true;
  }
  if (bx1 > PC.x1 && bx1 < PC.x3 && by2 > PC.y1 && by2 < PC.y3) {
    PowerUpCollected = true;
  }
  if (bx2 > PC.x1 && bx2 < PC.x3 && by1 > PC.y1 && by1 < PC.y3) {
    PowerUpCollected = true;
  }
...
void PC_Check_Collision_Enemy(int EnemyNumber) {
  ...
  if ((PCX2 > EX1 && PCX2 < EX2) || (PCX1 > EX1 && PCX1 < EX2)) {
    ...
    if ((PCY2 > EY1 && PCY2 < EY2) ||
        (PCY3 > EY1 && PCY3 < EY2))  // LOWER QUART OF PLAYER
    {
      if ((PC.JumpVelocity < 0 || PC.JumpVelocity == PC.JumpStrength) &&
          Enemy[EnemyNumber].Alive &&
          Enemy_Type[Enemy[EnemyNumber].Type].Stompable) {
        // STOMP THE ENEMY
    ...
    {
      if (Enemy[EnemyNumber].Alive && !PC.GodMode) {
        PC.gotKilled = true;
      }
    }
  }
