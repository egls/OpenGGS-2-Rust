...
void LOOP_Gameloop_Splashscreen() {
  ...
  while (!QuitProgram && !QuitToMenu && !ExitSplashscreen) {
    ...
    if (Key_ENTER_pressed || Key_CTRL_pressed || Joy_OK_pressed) {
      ExitSplashscreen = true;
    }
...
void LOOP_Gameloop_Splashscreen_Draw() {
  if (PC.Lives > 0) {
    ...
    if (PC.Stage < 10) {
      ...
      PrintInt(70 + Splashscreen.x_off, 32 + Splashscreen.y_off, 0, 0,
               PC.Stage);
    }
    if (PC.Stage > 9) {
      Print(Splashscreen.x_off, 32 + Splashscreen.y_off, 0, 0, "STAGE");
      PrintInt(60 + Splashscreen.x_off, 32 + Splashscreen.y_off, 0, 0,
               PC.Stage);
    }
  }
  if (PC.Lives < 1) {
