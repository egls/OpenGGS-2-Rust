...
void SPRITES_Draw() {
  int x;
  for (x = 0; x < 10; x++) {
    if (Sprite_SmallCoin[x].Active) {
      SPRITE_SmallCoin_Draw(x);
    }
    if (Sprite_PowerUp[x].Active) {
      SPRITE_PowerUp_Draw(x);
    }
  }
  if (Platform.inUse) {
    SPRITE_Platform_Draw();
  }

  for (x = 1; x < 30; x++) {
    if (Sprite_DropStone[x].inUse) {
      SPRITE_DropStone_Draw(x);
    }
  }
  ...
  if (Sprite_Bullet.ExplosionAnimation > 0) {
    SPRITE_Bullet_Explosion_Draw();
  }
}
...
void SPRITE_PowerUp_Draw(int PowerUpNumber) {
  ...
  SDL_RenderCopyEx(gRenderer, PowerUpTexture, &SrcRect, &DstRect, 0, NULL,
                   SDL_FLIP_NONE);
}
...
void SPRITE_SmallCoin_Draw(int SmallCoinNumber) {
  ...
  if (SmallCoinNumber ==
      0)  // DON'T MESS WITH SMALLCOIN[0] - IT'S USED IN THE INTERFACE
  {
...
void SPRITE_Bullet_Explosion_Draw() {
  ...
  SrcRect.x = 0;
  if (Sprite_Bullet.ExplosionAnimation < (Sprite_Bullet.ExplosionDuraion / 2)) {
    SrcRect.x = 32;
  }
  SrcRect.y = 272;
  SrcRect.w = 32;
...
void SPRITE_DropStone_Draw(int DropStoneNumber) {
  ...
  SrcRect.y = Sprite_DropStone[DropStoneNumber].y +
              StageC64.TileType * (10 * TS.Tile_Height);
  ;
