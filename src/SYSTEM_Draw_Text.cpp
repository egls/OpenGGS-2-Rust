...
void PrintInt_Big(int left, int top, int colour, double charDegree,
                  int number) {
  char Integer[] = "012345678901234567890123456789";
  // itoa(number,Integer,10);  // NOT GP2X compatible
  sprintf(Integer, "%d", number);  // GP2X compatible
...
void DrawChar(int left, int top, int colour, double charDegree, char letter) {
  ...
  if (colour < 4) {
    TextSrcRect.y += colour * 40;
  }

  SDL_RenderCopyEx(gRenderer, fontTexture, &TextSrcRect, &TextDstRect,
                   charDegree, NULL, SDL_FLIP_NONE);
  letter = ' ';
}
