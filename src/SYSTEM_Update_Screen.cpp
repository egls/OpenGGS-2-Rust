...
void Update_Screen() {
  ...
  if (SDL_GetTicks() - timecounter_Update_Screen > 10) {
    ...
    if (GV.Mode == MODE_EDITOR_STAGES) {
      drawGround(0);
      LOOP_Editor_Stages_Draw_Stage();
      LOOP_Editor_Stages_Draw_Enemies();
      LOOP_Editor_Stages_Draw_Interface();
    }
    if (GV.Mode == MODE_EDITOR_TILES) {
      drawGround(0);
      LOOP_Editor_Tiles_Draw();
      LOOP_Editor_Tiles_Interface_Draw();
    }
    if (GV.Mode == MODE_CREDITS) {
      drawGround(0);
      LOOP_Credits_Draw();
    }
    if (GV.Mode == MODE_OPTIONS) {
      drawGround(0);
      LOOP_Options_Draw();
    }
    if (GV.Mode == MODE_STORY) {
      drawGround(0);
      LOOP_Story_Draw();
    }
    if (GV.Mode == MODE_SPLASHSCREEN) {
      drawGround(1);
      LOOP_Gameloop_Splashscreen_Draw();
    }
    if (GV.Mode == MODE_WARPSCREEN) {
      drawGround(1);
      LOOP_Gameloop_Warpscreen_Draw();
    }
    if (GV.Mode == MODE_STAGEFILE_LOAD) {
      drawGround(0);
      LOOP_Stagefile_Load_Draw();
    }
