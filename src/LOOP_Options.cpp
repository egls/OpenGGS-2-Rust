...
void LOOP_Options() {
  ...
  while (!QuitProgram && !QuitToMenu) {
    ...
    if (Key_RIGHT_pressed || Joy_RIGHT_pressed) {
      ...
      if (Menu.ActiveOptions > Menu.MenuEntriesOptions - 1) {
        Menu.ActiveOptions = Menu.MenuEntriesOptions - 1;
      }
    }
    ...
    if (Key_DOWN_pressed || Joy_DOWN_pressed) {
      AUDIO_Sound_Play(AUDIO_CLICK);
      if (Menu.ActiveOptions < Menu.MenuEntriesOptions - Menu.Cols) {
        Menu.ActiveOptions = Menu.ActiveOptions + Menu.Cols;
      }
    }

    if (Key_UP_pressed || Joy_UP_pressed) {
      AUDIO_Sound_Play(AUDIO_CLICK);
      if (Menu.ActiveOptions > Menu.Cols - 1) {
        Menu.ActiveOptions = Menu.ActiveOptions - Menu.Cols;
      }
    }

    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 0 && GV.Resolution != RESOLUTION_640x480) {
      SYSTEM_SetResolution(RESOLUTION_640x480);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 1 && GV.Resolution != RESOLUTION_800x600) {
      SYSTEM_SetResolution(RESOLUTION_800x600);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 2 && GV.Resolution != RESOLUTION_1280x720) {
      SYSTEM_SetResolution(RESOLUTION_1280x720);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 3) {
      toggleScreenmode();
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 4) {
      AUDIO_Volume_Change_Music(VolumePercentage_Music += 10, true);
      AUDIO_Sound_Play(AUDIO_CLICK);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 5) {
      AUDIO_Volume_Change_Music(VolumePercentage_Music -= 10, true);
      AUDIO_Sound_Play(AUDIO_CLICK);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 6) {
      AUDIO_Volume_Change_Sound(VolumePercentage_Sound += 10, true);
      AUDIO_Sound_Play(AUDIO_CLICK);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 7) {
      AUDIO_Volume_Change_Sound(VolumePercentage_Sound -= 10, true);
      AUDIO_Sound_Play(AUDIO_CLICK);
    }
    if (!Key_LALT && (Key_ENTER_pressed || Joy_OK_pressed) &&
        Menu.ActiveOptions == 8) {
      Mix_HaltMusic();
