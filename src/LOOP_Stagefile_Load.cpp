...
void LOOP_Stagefile_Load() {
  ...
  while (!QuitProgram && !QuitToMenu) {
    ...
    if (Key_RIGHT_pressed || Joy_RIGHT_pressed) {
      ...
      if (Menu.ActiveFileLoad > Menu.MenuEntriesFileLoad - 1) {
        Menu.ActiveFileLoad = Menu.MenuEntriesFileLoad - 1;
      }
    }
    ...
    if (Key_DOWN_pressed || Joy_DOWN_pressed) {
      AUDIO_Sound_Play(AUDIO_CLICK);
      if (Menu.ActiveFileLoad < Menu.MenuEntriesFileLoad - Menu.Cols) {
        Menu.ActiveFileLoad = Menu.ActiveFileLoad + Menu.Cols;
      }
    }

    if (Key_UP_pressed || Joy_UP_pressed) {
      AUDIO_Sound_Play(AUDIO_CLICK);
      if (Menu.ActiveFileLoad > Menu.Cols - 1) {
        Menu.ActiveFileLoad = Menu.ActiveFileLoad - Menu.Cols;
      }
    }

    if (!Key_LALT) {
      if (Key_ENTER_pressed || Joy_OK_pressed) {
        GV.RandomLevels = false;
        Load_Stagefile(Menu.ActiveFileLoad + 1);
      }
    }
...
void Load_Stagefile(int StagefileNumber) {
  if (strncmp("---", FileName.Stage[StagefileNumber], 2) != 0) {
    snprintf(FileName.Stage[0], sizeof(char) * 128,
             FileName.Stage[StagefileNumber]);
