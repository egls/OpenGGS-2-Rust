...
void PC_Draw(int x, int y) {
  ...
  SDL_RenderCopyEx(gRenderer, PlayerTexture, &SrcRect, &DstRect, 0, NULL,
                   SDL_FLIP_NONE);

  if (PC.ShowBorder) {
    SDL_SetRenderDrawColor(gRenderer, 50, 50, 50, 255);
    SDL_RenderDrawLine(gRenderer, DstRect.x + int(SrcRect.w / 2), y - 10,
                       DstRect.x + int(SrcRect.w / 2), y + 10);
