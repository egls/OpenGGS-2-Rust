...
void PC_Jump() {
  ...
  if (PC.JumpOnGoing && (PC.OnGround || PC.OnGroundDelay > 0)) {
    PC.JumpVelocity =
        PC.JumpStrength;  // COULD BE MADE DEPENDENT ON WALKINGSPEED
    PC.OnGround = false;
    PC.OnGroundDelay = 0;
...
void PC_Gravity() {
  ...
  if (PC.JumpVelocity < -World.TerminalVelocity) {
    PC.JumpVelocity = -World.TerminalVelocity;
  }
  ...
  if (!PC.Dead && !Unified.Jump && PC.JumpVelocity > 0) {
