...
void SPRITE_Bullet_Move() {
  if (PC.TargetEnemy == 0) {
    if (Sprite_Bullet.Direction == PC_RIGHT) {
      Sprite_Bullet.RunVelocity += 1 * World.Friction;
    }
    if (Sprite_Bullet.Direction == PC_LEFT) {
      Sprite_Bullet.RunVelocity -= 1 * World.Friction;
    }
    ...
    // MAX SPEED
    if (Sprite_Bullet.RunVelocity < -Sprite_Bullet.RunStrength) {
      Sprite_Bullet.RunVelocity = -Sprite_Bullet.RunStrength;
    }  // MAX SPEED
    if (Sprite_Bullet.RunVelocity > Sprite_Bullet.RunStrength) {
      Sprite_Bullet.RunVelocity = Sprite_Bullet.RunStrength;
    }  // MAX SPEED
    ...
    if (Sprite_Bullet.VerticalDirection == 1) {
      Sprite_Bullet.PosY += 2;
    }
    if (Sprite_Bullet.VerticalDirection == 0) {
      Sprite_Bullet.PosY -= 2;
    }
    ...
    if (Sprite_Bullet.Active && PC.PowerUp == 2) {
      if (Sprite_Bullet.Direction == PC_RIGHT &&
          Sprite_Bullet_Collision_Right()) {
        Sprite_Bullet.Active = false;
      }
      if (Sprite_Bullet.Direction == PC_LEFT &&
          Sprite_Bullet_Collision_Left()) {
        Sprite_Bullet.Active = false;
      }
      if (Sprite_Bullet.VerticalDirection == 1 &&
          Sprite_Bullet_Collision_Bottom()) {
        Sprite_Bullet.VerticalDirection = 0;
      }
      if (Sprite_Bullet.VerticalDirection == 0 &&
          Sprite_Bullet_Collision_Top()) {
        Sprite_Bullet.VerticalDirection = 1;
      }
    }
    ...
    if (Sprite_Bullet.Active && PC.PowerUp > 2) {
      if (Sprite_Bullet.Direction == PC_RIGHT &&
          Sprite_Bullet_Collision_Right()) {
        Sprite_Bullet.RunVelocity = -Sprite_Bullet.RunVelocity;
        Sprite_Bullet.Direction = PC_LEFT;
      }
      if (Sprite_Bullet.Direction == PC_LEFT &&
          Sprite_Bullet_Collision_Left()) {
        Sprite_Bullet.RunVelocity = -Sprite_Bullet.RunVelocity;
        Sprite_Bullet.Direction = PC_RIGHT;
      }
      if (Sprite_Bullet.VerticalDirection == 1 &&
          Sprite_Bullet_Collision_Bottom()) {
        Sprite_Bullet.VerticalDirection = 0;
  ...
  // HOMING BULLET
  if (PC.TargetEnemy > 0) {
    if (Sprite_Bullet.PosX < Enemy[PC.TargetEnemy].PosX) {
      Sprite_Bullet.PosX += 6;
    }
    if (Sprite_Bullet.PosX > Enemy[PC.TargetEnemy].PosX) {
      Sprite_Bullet.PosX -= 6;
    }
    if (Sprite_Bullet.PosY <
        Enemy[PC.TargetEnemy].PosY -
            Enemy_Type[Enemy[PC.TargetEnemy].Type].ColHeight) {
      Sprite_Bullet.PosY += 6;
    }
    if (Sprite_Bullet.PosY >
        Enemy[PC.TargetEnemy].PosY -
            Enemy_Type[Enemy[PC.TargetEnemy].Type].ColHeight) {
  ...
  if (Sprite_Bullet.PosY < 0 || Sprite_Bullet.PosY > GV.Screen_Height + 32 ||
...
bool Sprite_Bullet_Collision_Right() {
  ...
  if (TileType
          [StageC64.TileNumber
               [(int)((Sprite_Bullet.PosX + Sprite_Bullet.w) / TS.Tile_Width)]
               [(int)((Sprite_Bullet.PosY + Sprite_Bullet.h) / TS.Tile_Height) -
                1]]
  ...
  if (TileType[StageC64.TileNumber[(
                   int)((Sprite_Bullet.PosX + Sprite_Bullet.w) / TS.Tile_Width)]
                                  [(int)(Sprite_Bullet.PosY / TS.Tile_Height)]]
          .Solid)  // CHECK TOP-RIGHT
  {
    ReturnValue = true;
  }
...
bool Sprite_Bullet_Collision_Left() {
  ...
  if (TileType[StageC64
                   .TileNumber[(int)(Sprite_Bullet.PosX / TS.Tile_Width)]
                              [(int)((Sprite_Bullet.PosY + Sprite_Bullet.h) /
                                     TS.Tile_Height) -
                               1]]
  ...
  if (TileType[StageC64.TileNumber[(int)(Sprite_Bullet.PosX / TS.Tile_Width)]
                                  [(int)(Sprite_Bullet.PosY / TS.Tile_Height)]]
...
bool Sprite_Bullet_Collision_Bottom() {
  ...
  // CHECK FOR WALLS
  if (Sprite_Bullet.Direction == PC_RIGHT) {
    XPOS = (int)((Sprite_Bullet.PosX + Sprite_Bullet.w) / TS.Tile_Width);
  }

  else {
    XPOS = (int)(Sprite_Bullet.PosX / TS.Tile_Width);
  }
...
void Sprite_Bullet_Collision_Enemies(int EnemyNumber) {
  ...
  int EX1 = Enemy[EnemyNumber].PosX -
            Enemy_Type[Enemy[EnemyNumber].Type].ColWidthHalf;
  int EX2 = Enemy[EnemyNumber].PosX +
            Enemy_Type[Enemy[EnemyNumber].Type].ColWidthHalf;
  int EY1 =
      Enemy[EnemyNumber].PosY - Enemy_Type[Enemy[EnemyNumber].Type].ColHeight;
  int EY2 = Enemy[EnemyNumber].PosY;

  if ((PCX2 > EX1 && PCX2 < EX2) || (PCX1 > EX1 && PCX1 < EX2)) {
    if ((PCY2 > EY1 && PCY2 < EY2) ||
        (PCY1 > EY1 && PCY1 < EY2))  // FULL HEIGHT OF PLAYER
    {
...
void Sprite_Bullet_Get_TargetEnemy() {
  ...
  for (i = 0; i < MAX_NUM_ENEMIES; i++) {
    if (Enemy[i].Active && Enemy[i].Alive &&
        Enemy_Type[Enemy[i].Type].Shootable) {
      if (PC.Direction == PC_LEFT) {
        EnemyDistance[i] = Enemy[i].PosX - PC.PosX;
      }
      if (PC.Direction == PC_RIGHT) {
        EnemyDistance[i] = PC.PosX - Enemy[i].PosX;
      }
      if (EnemyDistance[i] < ShortestDistance) {
        ShortestDistance = EnemyDistance[i];
        PC.TargetEnemy = i;
      }
    }
  }
