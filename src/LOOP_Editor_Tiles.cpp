...
void LOOP_Editor_Tiles() {
  ...
  if (GV.Resolution != RESOLUTION_640x480) {
    SYSTEM_SetResolution(RESOLUTION_640x480);
  }
  ...
  while (!QuitProgram && !QuitToMenu) {
    ...
    if (SDL_GetTicks() - timecounter_Scrolling > 5) {
      ...
      if (TE.PosY < GV.Screen_Height - TS.Height_Pixels) {
        TE.PosY = GV.Screen_Height - TS.Height_Pixels;
      }
    }
...
void LOOP_Editor_Tiles_Interface_Draw() {
  ...
  if (TE.MarkCoinBlocks) {
    Print(10, 70, 0, 0, ">");
  }
  if (TE.MarkBreakables) {
    Print(10, 82, 0, 0, ">");
  }
  ...
  if (TE.MarkPowerUpBlocks) {
    Print(180, 10, 0, 0, ">");
  }
  if (TE.MarkCoinBlocksHelmet) {
    Print(180, 22, 0, 0, ">");
  }
  if (TE.MarkColourChanging) {
    Print(180, 34, 0, 0, ">");
  }
  if (TE.MarkSubStageEntrance) {
    Print(180, 46, 0, 0, ">");
  }
  if (TE.MarkDropStone) {
    Print(180, 58, 0, 0, ">");
...
void LOOP_Editor_Tiles_Draw() {
  ...
  for (j = 0; j < TS.Height; j++) {
    for (i = 0; i < TS.Width; i++) {
      k = i + (j * TS.Width);
      if (TE.MarkSolids && !TileType[k].Solid) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY, INTERFACE_MARK_SOLID);
      }
      if (TE.MarkExits && TileType[k].Exit) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY, INTERFACE_MARK_EXIT);
      }
      if (TE.MarkLethals && TileType[k].Lethal) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_LETHAL);
      }
      if (TE.MarkFire && TileType[k].Fire) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY, INTERFACE_MARK_FIRE);
      }
      if (TE.MarkCoins && TileType[k].Coin) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY, INTERFACE_MARK_COIN);
      }
      if (TE.MarkCoinBlocks && TileType[k].CoinBlock) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_COIN_BLOCK);
      }
      if (TE.MarkBreakables && TileType[k].Breakable) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_BREAKABLE);
      }
      if (TE.MarkPowerUpBlocks && TileType[k].PowerUpBlock) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_POWERUP_BLOCK);
      }
      if (TE.MarkCoinBlocksHelmet && TileType[k].CoinBlockHelmet) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_COIN_BLOCK);
      }
      if (TE.MarkColourChanging && TileType[k].ColourChanging) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_COLOUR_CHANGING);
      }
      if (TE.MarkSubStageEntrance && TileType[k].SubStageEntrance) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY, INTERFACE_MARK_EXIT);
      }
      if (TE.MarkDropStone && TileType[k].DropStone) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_DROPSTONE);
      }
      if (TE.MarkWarpStone && TileType[k].WarpStone) {
        INTERFACE_Tile_Draw((i * TS.Tile_Width) + TE.PosX,
                            j * TS.Tile_Height + TE.PosY,
                            INTERFACE_MARK_WARPSTONE);
      }
    }
  }
...
void Mark_Tiles() {
  ...
  if (TE.MarkSolids) {
    TileType[TileNumber].Solid = !TileType[TileNumber].Solid;
  }
  if (TE.MarkExits) {
    TileType[TileNumber].Exit = !TileType[TileNumber].Exit;
  }
  if (TE.MarkLethals) {
    TileType[TileNumber].Lethal = !TileType[TileNumber].Lethal;
  }
  if (TE.MarkFire) {
    TileType[TileNumber].Fire = !TileType[TileNumber].Fire;
  }
  if (TE.MarkCoins) {
    TileType[TileNumber].Coin = !TileType[TileNumber].Coin;
  }
  if (TE.MarkCoinBlocks) {
    TileType[TileNumber].CoinBlock = !TileType[TileNumber].CoinBlock;
  }
  if (TE.MarkBreakables) {
    TileType[TileNumber].Breakable = !TileType[TileNumber].Breakable;
  }
  if (TE.MarkPowerUpBlocks) {
    TileType[TileNumber].PowerUpBlock = !TileType[TileNumber].PowerUpBlock;
  }
  if (TE.MarkCoinBlocksHelmet) {
    TileType[TileNumber].CoinBlockHelmet =
        !TileType[TileNumber].CoinBlockHelmet;
  }
  if (TE.MarkColourChanging) {
    TileType[TileNumber].ColourChanging = !TileType[TileNumber].ColourChanging;
  }
  if (TE.MarkSubStageEntrance) {
    TileType[TileNumber].SubStageEntrance =
        !TileType[TileNumber].SubStageEntrance;
  }
  if (TE.MarkDropStone) {
    TileType[TileNumber].DropStone = !TileType[TileNumber].DropStone;
  }
  if (TE.MarkWarpStone) {
    TileType[TileNumber].WarpStone = !TileType[TileNumber].WarpStone;
  }
...
void Tilesheet_Save() {
  ...
  for (j = 0; j < TS.Height; j++) {
    for (i = 0; i < TS.Width; i++) {
      ...
      TS.CoinBlockHelmet[k] =
          TileType[k].CoinBlockHelmet;                    // COPY INFOS INTO TS
      TS.ColourChanging[k] = TileType[k].ColourChanging;  // COPY INFOS INTO TS
      TS.SubStageEntrance[k] =
          TileType[k].SubStageEntrance;         // COPY INFOS INTO TS
