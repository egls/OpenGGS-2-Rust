...
void LOOP_Gameloop_Standard() {
  ...
  while (!QuitProgram && !QuitToMenu) {
    ...
    if (Key_F9_pressed) {
      GV.PixelPerfectRunning = !GV.PixelPerfectRunning;
    }

    if (Key_TAB_pressed) {}

    if (Key_CTRL_pressed || Joy_SHOOT) {
      SPRITE_Bullet_Activate();
    }
    ...
    if (PC.Exit_Reached) {
      if (PC.Stage == 33) {
        ...
        PC.Stage = PC.SubStageEntryStage;
        STAGE_Load(PC.Stage, 3, false, false);
      }  // EXITING A SUBSTAGE
      else if (PC.Stage < 33) {
        PC.Stage++;
        STAGE_Load(PC.Stage, 0, true, true);
      }  // NORMAL STAGE
    }

    if (PC.WarpCounter == 1) {
      LOOP_Gameloop_Warpscreen();
    }
    ...
    if (!PC.Dead && PC.WarpCounter == 0) {
      if (Key_UP_pressed || Joy_JUMP_pressed) {
        PC.JumpOnGoing = true;
      }
      if (Key_LEFT || Joy_LEFT) {
        PC.AccellLeft = true;
      }
      if (Key_RIGHT || Joy_RIGHT) {
        PC.AccellRight = true;
      }
      if (!Key_LEFT && !Joy_LEFT) {
        PC.AccellLeft = false;
      ...
      if (Key_LEFT_pressed || Key_RIGHT_pressed || Joy_LEFT_pressed ||
          Joy_RIGHT_pressed) {
        PC_Run();
      }
    }
    ...
    if (SDL_GetTicks() - timecounter > 20) {
      ...
      if (PC.JumpOnGoing || PC.Dead) {
        PC_Jump();
      }
      if (!Key_LEFT_pressed && !Key_RIGHT_pressed && !PC.Dead &&
          PC.WarpCounter == 0) {
        PC_Run();
      }
      ...
      if (!PC.OnGround && PC.PosY < GV.Screen_Height) {
        PC.Stance = PC_JUMP;
      }  // E.G. WALKING OFF A CLIFF
      if (PC.AccellLeft && PC.RunVelocity > 0) {
        PC.Stance = PC_TURN;
      }
      if (PC.AccellRight && PC.RunVelocity < 0) {
        PC.Stance = PC_TURN;
      }

      if (PC.Lives < 1 && !PC.Dead) {
        PC_Define();
        STAGE_Load(PC.Stage, 0, false, false);
        QuitToMenu = true;
      }  // LOST ALL LIVES. RESET PC AND GO TO MENU
    }
